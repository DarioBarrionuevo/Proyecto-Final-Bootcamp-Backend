...
module.exports = {
  ...
  organizationLogin: async function (req, res) {
    try {
      ...
      const _id = organizationData[0]._id;
      console.log("organizationData", _id);
      if (!organizationData) {
        res.status(401).json({
  ...
  addDeliveryPointToOrganization: async function (req, res) {
    try {
      const {
        delivery_points
      } = req.body;
      console.log("delivery_points", delivery_points)
  ...
  deleteDeliveryPointToOrganization: async function (req, res) {
    try {
      // traigo el index por el body
      const {
        index
      } = req.body;
      ...
      const organizationToOperateWith = await OrganizationModel.findById(req.params.id);
      const itemToDelete = organizationToOperateWith.delivery_points[index];

      const oneOrganization = await OrganizationModel.findByIdAndUpdate(
        req.params.id, {
          $pull: {
            "delivery_points": itemToDelete
          }
        }, {
          runValidators: true,
        }
      );
      // console.log("oneOrganization", oneOrganization)
      res.status(200).json({
        message: "Delivery point deleted ",
        organizationInfo: req.body,
      });
